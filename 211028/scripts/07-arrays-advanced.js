...
const scoresResult = {}
scores.forEach((data) => {
  scoresResult[data.name] = data.score
})
...
const bal = accounts.filter((account) => {
  return account.balance == 24000
})
const bal2 = accounts.find((account) => {
  return account.balance == 24000
})
...
const requestsResult = requests.some((req) => {
...
const usersResult = users.every((user) => {
